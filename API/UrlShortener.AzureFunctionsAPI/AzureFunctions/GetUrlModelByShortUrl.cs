...
{
    public class GetUrlModelByShortUrl
    {
        private readonly IUrlManager _urlManager;
        public GetUrlModelByShortUrl(IUrlManager urlManager)
        {
            _urlManager = urlManager;
        }

        [FunctionName(nameof(GetUrlModelByShortUrl))]
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "v1.0/GetByShortUrl/")] HttpRequest req,
            ILogger log)
        {
            try
            {
